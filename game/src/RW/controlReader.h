...
public:
    ...
    {
        if (keyStr == "pointer_down")
            return Key::pointer_down;
        if (keyStr == "pointer_left")
            return Key::pointer_left;
        if (keyStr == "pointer_up")
            return Key::pointer_up;
        if (keyStr == "pointer_right")
            return Key::pointer_right;
        if (keyStr == "main_action")
            return Key::main_action;
        if (keyStr == "extra_action_0")
            return Key::extra_action_0;
        if (keyStr == "extra_action_1")
            return Key::extra_action_1;
        if (keyStr == "save_action")
            return Key::save_action;
        if (keyStr == "load_action")
