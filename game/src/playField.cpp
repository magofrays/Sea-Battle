...
playField::playField(const json &data)
{ // we trust json
...
void playField::placeShip(std::shared_ptr<Ship> ship, shipManager &ship_manager)
{
    ...
    if (area.contains(ship_area) && !(ship_manager.shipIntersection(ship_area)))
    {
        ...
        if (ship->IsVertical())
        {
            ...
            int x = ship_area.min_point.x;
            int y_min = ship_area.min_point.y;
        ...
        {
            int y = ship_area.min_point.y;
            int x_min = ship_area.min_point.x;
