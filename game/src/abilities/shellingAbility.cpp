...
void shellingAbility::apply(humanPlayer * player){
    ...
    if(ship_manager->allShipsDestroyed()){
        player->Handle(textMessage("All ships are already destroyed!", {255, 255, 0}, textPosition::log).clone());
        return;
    }
    player->Handle(textMessage("Shelling ability applied!", {255, 0, 255}, textPosition::log).clone());
    ...
    player->Handle(textMessage("One of the ships was attacked!", {255, 0, 255}, textPosition::log).clone());
}
